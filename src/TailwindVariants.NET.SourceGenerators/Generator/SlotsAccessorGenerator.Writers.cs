...
public partial class SlotsAccessorGenerator
{
	...
	private static void WriteExtensions(Indenter sb, SlotsAccessorToGenerate accessor)
	{
		...
		sb.AppendLine(
			$"public static string? Get{accessor.ComponentTypeParameters}" +
			$"(this {accessor.SlotsMapName} slots, {enumRef} key)"
		);

		sb.Indent();
		sb.AppendLine($"{accessor.ComponentConstraints}");
		sb.Dedent();
		...
		foreach (var property in accessor.AllProperties)
		{
			...
			sb.AppendLine(
				$"public static string? Get{safe}{accessor.ComponentTypeParameters}" +
				$"(this {accessor.SlotsMapName} slots)"				
			);
	...
	{
		ImmutableArray<(string Name, string Modifiers)> hierarchy = accessor.Hierarchy;
	...
	private static void WritePreamble(Indenter sb, SlotsAccessorToGenerate accessor)
	{
		...
		if (!string.IsNullOrEmpty(accessor.NamespaceName) &&
			accessor.NamespaceName != "<global namespace>")
		{
